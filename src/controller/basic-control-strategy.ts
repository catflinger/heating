...
export class BasicControlStrategy implements IControlStrategy {
    ...
    public calculateControlState(currentState: Snapshot): ControlStateSnapshot {
        ...
        // the minimum temp
        const hwTemperature: number = this.getHwTemp(currentState.environment);
        ...
        currentState.controller.overrides.forEach((ov) => {
    ...
    private getHwTemp(environment: EnvironmentSnapshot): number {
        return environment.sensors.find((s) => s.id === "hw").reading;
    }
}
