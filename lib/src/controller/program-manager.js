"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
var fs = require("fs");
var inversify_1 = require("inversify");
var path = require("path");
var uuid_1 = require("uuid");
var types_1 = require("./types");
var ProgramManager = (function () {
    function ProgramManager(settings, programFactory) {
        this.settings = settings;
        this.programFactory = programFactory;
        this.ext = ".json";
        this._activeProgram = undefined;
        this._activeProgram = this.programFactory();
    }
    Object.defineProperty(ProgramManager.prototype, "activeProgram", {
        get: function () {
            return this._activeProgram;
        },
        enumerable: true,
        configurable: true
    });
    ProgramManager.prototype.list = function () {
        var _this = this;
        var results = [];
        var files = fs.readdirSync(this.settings.programStoreDir);
        var ids = [];
        files.forEach(function (f) {
            if (f.endsWith(_this.ext)) {
                ids.push(f.substr(0, f.length - _this.ext.length));
            }
        });
        ids.forEach(function (id) {
            results.push(_this.get(id));
        });
        return results;
    };
    ProgramManager.prototype.get = function (id) {
        var result = null;
        try {
            var json = fs.readFileSync(this.makePath(id), "utf8");
            result = this.programFactory();
            result.loadFrom(json);
        }
        catch (_a) {
            result = null;
        }
        return result;
    };
    ProgramManager.prototype.save = function (program) {
        var result = null;
        try {
            var id = program.id;
            if (!id) {
                id = uuid_1.v4();
            }
            fs.writeFileSync(this.makePath(id), program.toStorable());
            result = id;
        }
        catch (_a) {
            result = null;
        }
        return result;
    };
    ProgramManager.prototype.remove = function (id) {
        if (this._activeProgram != null && this._activeProgram.id !== id) {
            var filepath = this.makePath(id);
            if (fs.existsSync(filepath)) {
                fs.unlinkSync(filepath);
            }
        }
        else {
        }
    };
    ProgramManager.prototype.makePath = function (id) {
        return path.format({
            ext: this.ext,
            name: id,
            root: this.settings.programStoreDir,
        });
    };
    ProgramManager = __decorate([
        inversify_1.injectable(),
        __param(0, inversify_1.inject(types_1.INJECTABLES.ControllerSettings)),
        __param(1, inversify_1.inject(types_1.INJECTABLES.ProgramFactory)),
        __metadata("design:paramtypes", [Object, Function])
    ], ProgramManager);
    return ProgramManager;
}());
exports.ProgramManager = ProgramManager;
