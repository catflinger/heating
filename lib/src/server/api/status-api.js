    ...
    StatusApi.prototype.addRoutes = function (router) {
        ...
        router.get("/status", function (req, res, next) {
            ...
            try {
                ...
                var result = { items: [] };
                result.items.push(_this.controlResponse);
                result.items.push(_this.deviceResponse);
                result.items.push(_this.envResponse);
                result.items.push(_this.overrideResponse);
                result.items.push(_this.programResponse);
        ...
        router.delete("/status/*", function (req, res, next) {
            res.status(405).send("method not allowed");
        });
    };
